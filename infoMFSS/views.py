    ...
    def get_context_data(self, **kwargs):
        ...
        try:
            ...
            for mine in mines:
                ...
                context[f'percent_{mine.lower().replace(" ", "_").replace("нефтешахта_№", "mine")}'] = result[
                    "percent"
                ]
...
class EquipmentListView(LoggingMixin, LoginRequiredMixin, FormView):
    ...
    def form_valid(self, form):
        ...
        """
        params = {
            "mine": form.cleaned_data.get(
                "number_mines",
            ),
            "subsystem": form.cleaned_data.get(
                "subsystems",
            ),
            "incl_blocks": form.cleaned_data.get(
                "incl_blocks",
            ),
            "equipment": form.cleaned_data.get(
                "equipment",
            ),
        }
            ...
            context.update(
                {
                    ...
                    "subsystem": filter_params.get(
                        "subsystem",
                    ),
                    "incl_blocks": filter_params.get(
                        "incl_blocks",
                    ),
                    "equipment": filter_params.get(
                        "equipment",
                    ),
                }
            )
